...
void GPIO::GenerateVariablePWM(int period, int first_pulse, int last_pulse, int num_periods) {
  ...
  this->GeneratePWM(period, first_pulse, period);

  int diff = (last_pulse - first_pulse) / num_periods;

  for (int x = first_pulse; x < last_pulse; x += diff) {
    this->GeneratePWM(period, x, num_periods);
  }
}
